...
| EVSE | `backend/simulator_core/evse.py` | Per-connector state machine and meter state (energy, power, SoC). |
| Meter engine | `backend/simulator_core/meter_engine.py` | Per-EVSE asyncio loop that updates meter state and sends MeterValues to the CSMS. |
